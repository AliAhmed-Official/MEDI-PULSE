...
function PharmacyHeaderSidebar() {
  ...
  const [isAppointmentListOpen, setIsAppointmentListOpen] = useState(() => {
    return JSON.parse(localStorage.getItem('isAppointmentListOpen')) || false;
  });
  const toggleAppointmentList = () => {
    setIsAppointmentListOpen((prev) => {
      const newState = !prev;
      localStorage.setItem('isAppointmentListOpen', JSON.stringify(newState));
      return newState;
    });
  };
  useEffect(() => {
    const storedState = JSON.parse(localStorage.getItem('isAppointmentListOpen'));
    if (storedState !== null) {
      setIsAppointmentListOpen(storedState);
    }
  }, []);
  return (
    <>
      <header className='fixed w-full z-40 bg-white'>
        ...
        <div className={`absolute right-3 top-[116px] md:top-[60px] ${isprofileDropMenuOpen ? '' : 'hidden'}`}>
          <div className='bg-[#EFEDF1] w-52 p-3 flex flex-col gap-5'>
            <Link to='/pharmacy/pharmacysettings'>
      ...
      <div className={` max-w-[260px] min-w-[260px] overflow-y-auto z-50 fixed mt-[60px] pt-7 items-center transition-transform duration-300 ease-in-out bg-white ${isSidebarOpen} lg:translate-x-0`} style={{ height: 'calc(100vh - 60px)' }}>
        <div className='flex flex-col items-center mb-5'>
          ...
          <div className='text-center space-y-1 mt-3'>
            <p className='font-medium'>Muhammad Usman</p>
        ...
        <div className='flex flex-col gap-2 text-gray-600 mb-3'>
          ...
          <div className='px-2'>
            <div className='p-3 rounded-lg text-xl flex items-center gap-2 hover:bg-[#F0F3FB] relative cursor-pointer' onClick={toggleAppointmentList}>
              ...
              <p className='text-lg'>Medicines</p>
              <i className={`fa-solid text-sm absolute right-3 ${isAppointmentListOpen ? 'fa-minus' : 'fa-plus'}`}></i>
            </div>
          </div>
          <div className={`px-2 space-y-2 ${isAppointmentListOpen ? 'block' : 'hidden'}`}>
            <NavLink to='/pharmacy/medicinelist' className={({ isActive }) => `p-2 rounded-lg text-xl cursor-pointer block ${isActive ? 'bg-[#F0F3FB]' : 'bg-transparent'}`}>
              <p className='text-base ml-14'>View Medicine Stock</p>
            </NavLink>
            <NavLink to='/pharmacy/addmedicine' className={({ isActive }) => `p-2 rounded-lg text-xl cursor-pointer block ${isActive ? 'bg-[#F0F3FB]' : 'bg-transparent'}`}>
              <p className='text-base ml-14'>Add Medicine</p>
            </NavLink>
          </div>

          <div className='px-2'>
            <NavLink to='/pharmacy/prescriptionlist' className={({ isActive }) => `p-3 rounded-lg text-xl flex items-center gap-2 hover:bg-[#F0F3FB] cursor-pointer ${isActive ? 'bg-[#F0F3FB]' : 'bg-transparent'}`}>
              <i className="fa-solid fa-file-lines w-6 text-center"></i>
              <p className='text-lg'>Prescription List</p>
            </NavLink>
          </div>

          <div className='px-2'>
            <NavLink to='/pharmacy/pharmacysettings' className={({ isActive }) => `p-3 rounded-lg text-xl flex items-center gap-2 hover:bg-[#F0F3FB] cursor-pointer ${isActive ? 'bg-[#F0F3FB]' : 'bg-transparent'}`}>
...
export default PharmacyHeaderSidebar;